...
A_t = [1.01, 0.01, 0; 
    ...
    0, 0.01, 1.01];
B_t = eye(3);
M_t = 10^(-3)*eye(3); %state cost wt
Omega_t = 1.5*eye(3); %noise covariance
N_t = eye(3); %control cost wt

T = 15; %time horizon

x_0 = [-1; -1; -1]; %initial state
...
%     ylim([-1, 3]);
    ...
    plot(linspace(0, T, T+1), x_t_all_LQG(3,:), '--r', 'Linewidth', 2);
...
%     ylim([-1, 3]);
    ...
    plot(linspace(0, T-1, T), u_t_all_LQG(3,:), '--r', 'Linewidth', 2);
